...
impl VideoEncoder {
    ...
    pub fn start(&mut self) -> Result<()> {
        ...
        self.encoder = Some(Arc::new(Mutex::new(VideoEncoderContext {
            encoder,
            output_context,
            video_stream_index,
        })));
    ...
    pub fn stop(&mut self) -> Result<()> {
        *self.is_encoding.lock().unwrap() = false;
        ...
        if let Some(encoder_context) = self.encoder.take() {
            // Write trailer
            let mut encoder_ctx = encoder_context.lock().unwrap();
            encoder_ctx.output_context.write_trailer()?;
        }
...
fn encoding_loop(
    ...
    while *is_encoding.lock().unwrap() {
        ...
        if !queue.is_empty() {
            ...
            // Encode frame with thread-safe access
            let mut encoder_ctx = encoder_context.lock().unwrap();
            encoder_ctx.encoder.send_frame(&video_frame)?;
            
            // Receive and write packets
            let mut encoded = ffmpeg_next::Packet::empty();
            while encoder_ctx.encoder.receive_packet(&mut encoded).is_ok() {
                encoded.set_stream(encoder_ctx.video_stream_index);
                encoded.write_interleaved(&mut encoder_ctx.output_context)?;
            }
            drop(encoder_ctx);
