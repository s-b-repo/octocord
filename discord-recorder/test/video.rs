...
use ffmpeg_next::{
    codec::{self, context::Context, encoder, Id},
    encoder::video::Video as FFmpegVideoEncoder,
    format::{context::Output as OutputContext, output, Pixel},
...
impl VideoEncoder {
    ...
    pub fn start(&mut self) -> Result<()> {
        ...
        // Create video stream
        let video_stream = output_context.add_stream(Id::H264)?;
        let video_stream_index = video_stream.index();

        // Find H.264 encoder
        let codec = encoder::find(Id::H264).ok_or_else(|| anyhow!("H.264 encoder not found"))?;

        // Create and configure codec context for encoder
        let mut enc_ctx = Context::new_with_codec(codec);

        // Set dimensions, timebase, framerate, bitrate, pixel format using API-compatible names
        enc_ctx.set_dimensions(self.width as i32, self.height as i32);
        let time_base = Rational::new(1, self.frame_rate as i32);
        enc_ctx.set_time_base(time_base);
        enc_ctx.set_frame_rate(Some(Rational::new(self.frame_rate as i32, 1)));
        enc_ctx.set_bit_rate(self.bitrate as i64);
        enc_ctx.set_pixel_format(Pixel::YUV420P);

        // Open encoder (bind codec context to the codec)
        // The open method is exposed via encoder() -> EncoderContext -> open_as
        // This sequence returns a Context that has been opened with codec parameters
        let opened_ctx = enc_ctx.encoder().open_as(codec)?;
        let video_encoder = opened_ctx.encoder().video()?;

        // Attach parameters to the stream from the opened context
        video_stream.set_parameters(&opened_ctx);
        ...
        // Keep encoder context in the struct and clone Arc for thread
...
fn encoding_loop(
    ...
    // Frame rate derived from time_base denominator (time_base = 1/framerate)
    let frame_rate = {
    ...
    while *is_encoding.lock().unwrap() {
        ...
        if let Some(frame) = queue.pop_front() {
            ...
            let mut yuv_frame = VideoFrame::new(Pixel::YUV420P, rgb_frame.width(), rgb_frame.height());
            enc_ctx.scaler.run(&rgb_frame, &mut yuv_frame)?;

            yuv_frame.set_pts(Some(enc_ctx.next_pts));
            enc_ctx.next_pts += 1; // increment per frame in time_base units
            ...
            while enc_ctx.encoder.receive_packet(&mut encoded).is_ok() {
                ...
                encoded.write_interleaved(&mut enc_ctx.output_context)?;
            }
            drop(enc_ctx);
...
fn convert_to_rgb_frame(image: DynamicImage) -> Result<VideoFrame> {
    // Convert to RGB24 buffer and create a VideoFrame with Pixel::RGB24
    let rgb_image = image.to_rgb8();
    let (width, height) = rgb_image.dimensions();

    let mut frame = VideoFrame::new(Pixel::RGB24, width, height);

    // Copy raw pixels into the frame buffer row-by-row
