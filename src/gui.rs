use anyhow::Result;
use egui::{CentralPanel, SidePanel, TopBottomPanel, RichText, Color32, Stroke, CornerRadius, StrokeKind};
...
impl eframe::App for DiscordRecorderApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        TopBottomPanel::top("controls_panel").show(ctx, |ui| {
            ui.horizontal_centered(|ui| {
                ...
                let button = ui.add_sized(
                    ...
                    .fill(button_color)
                    .corner_radius(8) // Changed: rounding -> corner_radius, 8.0 -> 8
                );

                let is_recording = state.is_recording;
                if button.clicked() {
                    drop(state);
                    if let Err(e) = if is_recording {
                        self.stop_recording()
                    } else {
                ...
                ui.horizontal(|ui| {
                    let (status_text, status_color) = if is_recording {
        ...
        CentralPanel::default().show(ctx, |ui| {
            ...
            ui.painter().rect(
                preview_rect,
                CornerRadius::same(8), // Changed: Rounding -> CornerRadius, 8.0 -> 8
                              Color32::from_rgb(30, 31, 34), // Discord dark background
                              Stroke::new(1.0, Color32::from_rgb(64, 68, 75)),
                              StrokeKind::Outside // Added: required parameter
            );
            ...
            // Show recording info
            ui.scope_builder(egui::UiBuilder::new().max_rect(preview_rect), |ui| { // Changed: allocate_new_ui -> scope_builder
