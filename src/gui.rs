...
impl DiscordRecorderApp {
    ...
    fn start_recording(&mut self) -> Result<()> {
        ...
        std::fs::create_dir_all(&state.output_path)?;
 
        let include_audio = state.record_audio;
        let include_video = state.record_video;
        let include_webcam = state.record_webcam;
        let separate_outputs = state.separate_outputs;
        let selected_screen = state.selected_screen;
        let audio_device_opt = state.selected_audio_device.clone();
        let webcam_device_opt = state.selected_webcam.clone();
        let audio_gain_db = state.audio_gain_db;
        let use_pipewire_on_wayland = state.use_pipewire_on_wayland;
 
        let mut config_snapshot = state.config.clone();
        config_snapshot.set_output_directory(state.output_path.clone());
        ...
        config_snapshot.enable_preview_overlay = state.enable_preview_overlay;
        config_snapshot.video_quality = state.video_quality;
        config_snapshot.audio_quality = state.audio_quality;
        ...
        state.config = config_snapshot.clone();
        let screen_index = selected_screen.unwrap_or(0);
        let _audio_device = audio_device_opt.clone().unwrap_or_else(|| "default".to_string());
        let _webcam_name = webcam_device_opt.clone().unwrap_or_else(|| "Default Webcam".to_string());
        ...
        let options = RecorderOptions {
            ...
            separate_outputs,
            selected_screen,
            audio_device: audio_device_opt,
            webcam_device: webcam_device_opt,
        ...
        if include_webcam {
            if let Some(capture) = self.webcam_capture.as_mut() { let _ = capture.stop(); }
            self.webcam_capture = None;
        }

        // Skip starting CPAL audio monitor during recording to reduce contention and lag.

        self.video_encoder = Some(VideoEncoder::new(options)?);
        if let Some(encoder) = self.video_encoder.as_mut() {
            if let Err(e) = encoder.start() { error!("Failed to start encoder: {}", e); return Err(e); }
        }
