...
fn pick_best_input_host() -> Host {
    ...
    // Prefer ALSA if available and have at least one input device, then others
...
impl AudioRecorder {
    ...
    where
        T: cpal::Sample + cpal::SizedSample + ToPrimitive + Copy + 'static,
    {
        let err_fn = move |err| {
        ...
        let stream = device.build_input_stream(
            ...
            move |data: &[T], _info: &cpal::InputCallbackInfo| {
                ...
                for &sample in data {
                    ...
                    let f: f32 = num_traits::ToPrimitive::to_f32(&sample).unwrap_or(0.0);
                    local.push(f);
                }
...
fn find_audio_device(host: &Host, device_name: &str) -> Result<Device> {
    if device_name.is_empty() || device_name.eq_ignore_ascii_case("default") {
