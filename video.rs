        ...
        if effective_include_webcam {
            ...
            if options.include_video {
                filter_complex = Some(format!(
                    "[{webcam}:v]scale=640:-1[cam_scaled];[0:v][cam_scaled]overlay=W-w-40:H-h-40[vout]",
                    webcam = webcam_index
                ));
...
fn determine_screen_input(screen_index: Option<usize>) -> Result<ScreenCaptureInput> {
    ...
    let image = screen.capture().context("Failed to capture screen to determine resolution")?;
